...
- テストインフラ → [Testing](#開発・テストインフラ)
- 開発セットアップとワークフロー → [Development Guide](#開発・テストインフラ)
...
このアプリケーションは、**React 19 によるクライアントサイドレンダリングの SPA アーキテクチャ**を採用しています。フロントエンドは Vite で構築され、認証・データベース操作・ファイルストレージのために Supabase（Backend-as-a-Service）と通信します。React Query は積極的なキャッシュ無効化戦略（`staleTime: 0`）でプライマリな状態管理ソリューションとして機能します。

...
File naming: `<PLAN_ROOT>/NNN-short-slug.md`, numbered in recommended execution order. Replace `<PLAN_ROOT>` with the concrete plan directory selected for the invocation; never leave the placeholder in a generated plan.
...
> report — do not improvise. Do not edit `<PLAN_ROOT>/README.md`; report the
> result to the reviewer, who owns index and status updates through
> `reconcile`.
>
> **Drift check (run first)**: `git diff --stat <planned-at SHA>..HEAD -- <in-scope paths>`
> Then inspect current working-tree drift separately:
> `git diff --stat -- <in-scope paths>` (unstaged),
> `git diff --cached --stat -- <in-scope paths>` (staged), and
> `git ls-files --others --exclude-standard -- <in-scope paths>` (untracked).
> Start implementation only from a clean working tree. If any in-scope file
> changed since this plan was written, compare the "Current state" excerpts
> against the live code before proceeding; on any in-scope mismatch or
> pre-existing working-tree change, treat it as a STOP condition.
...
- **Effort**: S | M | L
- **Risk**: LOW | MED | HIGH
- **Depends on**: <PLAN_ROOT>/NNN-*.md (or "none")
...
- [ ] Completion result reported to the reviewer; executor did not modify `<PLAN_ROOT>/README.md`
...
## Index file: `<PLAN_ROOT>/README.md`

Written once by the advisor after all plans and maintained by the advisor/reviewer through `reconcile`. Executors may read it for dependencies but must never modify it:
...
honor its STOP conditions, and report the result to the reviewer. The reviewer
updates this index through reconcile.
